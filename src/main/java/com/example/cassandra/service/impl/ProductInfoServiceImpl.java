...
//        }
        Observable.range(productInfoRequest.getBeginWinHead(), productInfoRequest.getEndWinHead())
                .flatMap(winHead -> Observable.range(productInfoRequest.getBeginWinTail(), productInfoRequest.getEndWinTail())
                        .map(winTail -> new ProductInfo(productInfoRequest.getProjCode(), productInfoRequest.getProdBatchCode(), String.format("%07d", winHead), String.format("%03d", winTail), String.format("%d等奖", winTail)))
                        .toList()
                        .toObservable())
